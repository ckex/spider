...
public class CarHomeNetService {
  ...
  public void writeToDb(String carHtml) {
    ...
    for (int i = 0; i < list.size(); i++) {
      ...
      // 如果已经存在就更新，否者插入
      if(car.getSourceCome().contains("汽车之家")){
        car2 = carBodyInfoDao.load(car.getUniqueCarBrand());
        if(car2 !=null){
           carBodyInfoDao.update(car);
        }else{
           carBodyInfoDao.create(car);
        }
      }else if(car.getSourceCome().contains("腾讯汽车")){
        car2 = carBodyInfoDao.loadTencent(car.getUniqueCarBrand());
        if(car2 !=null){
          carBodyInfoDao.updateTencent(car);
        }else {
          carBodyInfoDao.createTencent(car);
        }
      }else if(car.getSourceCome().contains("网易汽车")){
        car2 = carBodyInfoDao.loadAuto(car.getUniqueCarBrand());
        if(car2 !=null){
          carBodyInfoDao.updateAuto(car);
        }else {
          carBodyInfoDao.createAuto(car);
        }
      }
    }
  }
