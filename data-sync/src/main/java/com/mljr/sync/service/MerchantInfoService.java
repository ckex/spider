...
public class MerchantInfoService {
    ...
    try {
      Function<MerchantInfoDo, Boolean> function = new Function<MerchantInfoDo, Boolean>() {
        ...
        public Boolean apply(MerchantInfoDo merchantInfoDo) {
          ...
          return sentMercentInfo(rmq, merchantInfoDo);
        }
      };
  ...
  private boolean sentMercentInfo(Rmq rmq, MerchantInfoDo merchantInfoDo) {
    ...
    return rmq.publish(new java.util.function.Function<Channel, Boolean>() {
      ...
      public Boolean apply(Channel t) {
        try {
          RabbitmqClient.publishMessage(t, ServiceConfig.getMerchantInfoExchange(), ServiceConfig.getMerchantInfoRoutingKey(), builder.build(),
  ...
  private boolean sentMercentInfo(Channel channel, MerchantInfoDo merchantInfoDo) {
    ...
    builder.contentEncoding(BasicConstant.UTF8).contentType(BasicConstant.TEXT_PLAIN).deliveryMode(1).priority(0);
    try {
      RabbitmqClient.publishMessage(channel, ServiceConfig.getMerchantInfoExchange(), ServiceConfig.getMerchantInfoRoutingKey(), builder.build(),
          jsonString.getBytes(Charsets.UTF_8));
      try {
        TimeUnit.MILLISECONDS.sleep(10);
      } catch (InterruptedException e) {
