...
public class MerchantInfoService {
	...
	private static final String MERCHANT_INFO_EXIST_IDS_KEY = Joiner.on("-").join(BasicConstant.MERCENT_INFO,
	...
	private boolean sentMercentInfo(Rmq rmq, MerchantInfoDo merchantInfoDo) {
		...
		return rmq.publish(new java.util.function.Function<Channel, Boolean>() {
			...
			public Boolean apply(Channel t) {
				try {
					RabbitmqClient.publishMessage(t, ServiceConfig.getMerchantInfoExchange(),
							ServiceConfig.getMerchantInfoRoutingKey(), builder.build(),
