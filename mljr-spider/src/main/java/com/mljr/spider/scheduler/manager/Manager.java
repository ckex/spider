...
public class Manager extends AbstractMessage {
  ...
  private void startSaiGeGPS() throws Exception {
    ...
    final Spider spider =
        Spider.create(processor).addPipeline(htmlPipeline).thread(1).setDownloader(new RestfulDownloader()).setExitWhenComplete(false);
  ...
  private void startJuheMobile() throws Exception {
    ...
    final Spider spider = Spider.create(processor).addPipeline(htmlPipeline).setDownloader(new RestfulDownloader()).thread(MAX_SIZE + CORE_SIZE)
  ...
  private void startBaiduMobile() throws Exception {
    ...
    Pipeline htmlPipeline = new HttpPipeline(targetUrl, this.httpClient, pipeline);
    final Spider spider = Spider.create(processor).addPipeline(htmlPipeline).thread(MAX_SIZE + CORE_SIZE).setExitWhenComplete(false);
  ...
  private void startSogouMobile() throws Exception {
    ...
    Pipeline htmlPipeline = new HttpPipeline(targetUrl, this.httpClient, pipeline);
    final Spider spider = Spider.create(processor).addPipeline(htmlPipeline).thread(1).setExitWhenComplete(false);
  ...
  private void startLBSAMapReGeo() throws Exception {
    ...
    // Pipeline htmlPipeline = new HttpPipeline(targetUrl, this.httpClient, pipeline);
    final Spider spider = Spider.create(processor).addPipeline(pipeline).thread(MAX_SIZE + CORE_SIZE).setExitWhenComplete(false);
  ...
  private void startBlackIdCard() throws Exception {
    ...
    Pipeline htmlPipeline = new HttpPipeline(targetUrl, this.httpClient, pipeline);
    final Spider spider = Spider.create(processor).addPipeline(htmlPipeline).thread(5).setExitWhenComplete(false);
  ...
  private void startQQZoneIndex() throws Exception {
    ...
    Pipeline htmlPipeline = new HttpPipeline(targetUrl, this.httpClient, pipeline);
    final Spider spider =
        Spider.create(processor).addPipeline(htmlPipeline).setDownloader(new QQSeleniumDownloader()).thread(1).setExitWhenComplete(false);
  ...
  private void startAutohomeTargetUrls() throws Exception {
    ...
    final Spider spider =
        Spider.create(processor).addPipeline(new AutohomeTargetUrlsPipeline()).thread(MAX_SIZE + CORE_SIZE).setExitWhenComplete(false);
    spider.setExecutorService(DEFAULT_THREAD_POOL);
  ...
  //360卡车之家
  private void  startTruckCarHomeNet() throws Exception{
    PageProcessor processor = new TruckCarHomeProcessor();
    final Spider spider = Spider.create(processor)
            .addPipeline(new CarHomeNetInfoPipeline()).thread(5)
            .setExitWhenComplete(false);
    //spider.setExecutorService(DEFAULT_THREAD_POOL);
    final AbstractScheduler scheduler = new TruckCarHomeScheduler(spider, TRUCKCAR_FALG_QUEUE_ID);
    spider.setScheduler(scheduler);
    spider.runAsync();
    logger.info("Start startTruckCarHomeNet finished. " + spider.toString());
  }
  //腾讯汽车
  private void  startTenCentCarNet() throws  Exception{
    PageProcessor processor = new TencentCarNetworkProcessor();
    final Spider spider = Spider.create(processor)
            .addPipeline(new CarHomeNetInfoPipeline()).thread(5)
            .setExitWhenComplete(false);
    final AbstractScheduler scheduler = new TencentCarNetScheduler(spider,TENCENT_FALG_QUEUE_ID);
    spider.setScheduler(scheduler);
    spider.runAsync();
