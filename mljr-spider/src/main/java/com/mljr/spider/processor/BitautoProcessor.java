...
/**
 * Created by songchi Date:2016/12/29
 */
public class BitautoProcessor extends AbstractPageProcessor {

  private static final Site site = Site.me().setDomain(DomainConstant.DOMAIN_BITAUTO).setSleepTime(300).setRetrySleepTime(2000).setRetryTimes(1)
  ...
  public static void main(String[] args) throws Exception {
    ...
    Spider.create(new BitautoProcessor()).addUrl("http://price.bitauto.com/mb9/").setDownloader(phantomDownloader).addPipeline(collectorPipeline)
        .thread((Runtime.getRuntime().availableProcessors() - 1) << 1).run();
