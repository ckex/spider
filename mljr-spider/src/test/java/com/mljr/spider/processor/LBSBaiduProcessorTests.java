...
public class LBSBaiduProcessorTests {
  ...
  private static void testGeo() {
    ...
    Spider spider = Spider.create(new LBSBaiduGeoProcessor()).addUrl(url).addPipeline(new ConsolePipeline());
    spider.runAsync();
  }

  private static void testReGeo() {
    String url = String.format(REGEO_URL, BAIDU_KEY, "121.516350,31.229872");
    Spider spider = Spider.create(new LBSBaiduReGeoProcessor()).addUrl(url).addPipeline(new ConsolePipeline());
    spider.runAsync();
  }
}
