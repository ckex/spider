...
public class LBSAMapProcessorTests {
  ...
  private static void testGeo() {
    ...
    Spider spider = Spider.create(new LBSAMapGeoProcessor()).addUrl(url).addPipeline(new ConsolePipeline());
    spider.runAsync();
  }

  private static void testReGeo() {
    String url = String.format(REGEO_URL, AMAP_KEY, "121.516350,31.229872");
    Spider spider = Spider.create(new LBSAMapReGeoProcessor()).addUrl(url).addPipeline(new ConsolePipeline());
    spider.runAsync();
  }
